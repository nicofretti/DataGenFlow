...
def image_scaffold(
    ...
    The generated Dockerfile builds from source (must be run from the
    DataGenFlow repository root) and optionally installs additional
    dependencies declared in user block files.
    """
    deps: set[str] = set()
    ...
    dockerfile = """# Custom DataGenFlow image with user blocks
# Generated by: dgf image scaffold
# Build from DataGenFlow repository root:
#   docker build -f Dockerfile.custom -t my-datagenflow:latest .
...
RUN python -m compileall -b -q lib/
...
RUN mkdir -p /app/data /app/lib/blocks/custom /app/user_blocks /app/user_templates

"""

    if deps:
        dockerfile += "# Install custom block dependencies\n"
        dockerfile += "RUN uv pip install \\\n"
        dockerfile += " \\\n".join(f"    {dep}" for dep in sorted(deps))
        dockerfile += "\n\n"

    dockerfile += """ENV PATH="/app/.venv/bin:$PATH"
