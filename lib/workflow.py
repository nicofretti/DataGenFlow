...
class Pipeline:
    ...
    def _filter_output_data(
        self, accumulated_data: dict[str, Any], initial_data_keys: set[str]
    ) -> dict[str, Any]:
        """
        filter out input metadata from final output to keep only generated data fields

        removes template configuration fields (samples, target_count, etc.) that were
        merged for template rendering but shouldn't appear in final results
        """
        return {k: v for k, v in accumulated_data.items() if k not in initial_data_keys}

    async def _save_seed_result(
        self,
        initial_data: dict[str, Any],
        accumulated_data: dict[str, Any],
        initial_data_keys: set[str],
        ...
        # filter out input metadata before saving
        filtered_output = self._filter_output_data(accumulated_data, initial_data_keys)

        record = RecordCreate(
            metadata=initial_data, output=json.dumps(filtered_output), trace=trace
        )
    ...
    async def _process_single_seed(
        ...
        # merge initial_data with seed_data to preserve original metadata
        # initial_data contains template variables (fields_to_generate, comparison_fields, etc.)
        # seed_data contains skeleton fields generated by StructureSampler
        # seed fields override initial fields (so skeleton values take precedence)
        merged_state = {**initial_data, **seed_data}

        # track which fields were in initial_data (input metadata to filter out later)
        initial_data_keys = set(initial_data.keys())
        ...
        context = BlockExecutionContext(
            ...
            job_id=job_id,
            pipeline_id=pipeline_id,
            accumulated_state=merged_state,
            usage=pipeline.Usage(),
            trace=[],
            constraints=constraints,
        )
        ...
        try:
            ...
            # filter output data before saving or returning
            filtered_result = self._filter_output_data(context.accumulated_state, initial_data_keys)
