...
def test_json_generation_template_structure():
    ...
    assert template['name'] == 'JSON Extraction'
...
async def test_json_generation_template_renders_content(mock_llm):
    ...
    assert '{{ content }}' not in captured_prompt, "Template not rendered - still contains {{ content }}"
...
async def test_qa_generation_template_renders_content(mock_llm):
    ...
    seed_data = {
        'content': 'Photosynthesis is how plants convert sunlight.'
    }
    ...
    def capture_call(*args, **kwargs):
        ...
        if len(captured_prompts) == 1:
            ...
            mock_response.choices[0].message.content = '{"qa_pairs": [{"question": "What is photosynthesis?", "answer": "How plants convert sunlight."}]}'
    ...
    assert '{{ assistant }}' not in captured_prompts[1], "StructuredGenerator prompt not rendered - still has {{ assistant }}"
    assert '{{ content }}' not in captured_prompts[1], "StructuredGenerator prompt not rendered - still has {{ content }}"
