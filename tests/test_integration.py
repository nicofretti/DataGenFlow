...
from models import GenerationConfig, Record, RecordStatus, SeedInput
...
class TestWorkflowIntegration:
    ...
    async def test_pipeline_with_mocked_llm(self):
        ...
        with patch("lib.blocks.builtin.llm.Generator") as MockGenerator:
            ...
            mock_instance.generate = AsyncMock(
                return_value="This is a test response from LLM"
            )

            input_data = {"system": "You are helpful", "user": "Say hello"}
    ...
    async def test_pipeline_data_flow(self):
        ...
        input_data = {"text": "  hello   world  ", "metadata": {"test": "preserved"}}
...
class TestLegacyPipelineIntegration:
    ...
    async def test_legacy_pipeline_with_mocked_generation(self, storage):
        ...
        try:
            ...
            with patch.object(
                pipeline.generator, "generate", new_callable=AsyncMock
            ) as mock_generate:
...
class TestEndToEndWorkflow:
    ...
    async def test_complete_qa_generation_workflow(self, storage):
        ...
        with patch.object(
            llm_block.generator, "generate", new_callable=AsyncMock
        ) as mock_generate:
            ...
            input_data = {"system": filled_system, "user": filled_user}
    ...
    async def test_batch_processing_workflow(self, storage):
        ...
        for i, result in enumerate(results):
            record = Record(
                ...
                status=(
                    RecordStatus.ACCEPTED if result["valid"] else RecordStatus.REJECTED
                ),
            )
